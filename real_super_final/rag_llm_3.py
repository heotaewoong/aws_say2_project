...
2. top1_reasoning: Explicitly mention all Positive HPO, Negative HPO, and abnormal Lab values with specific numbers.
...
class RareLinkHybridDualRAG:
    def __init__(self, orphadata_csv_path, hpo_dict_path="aws_say2_project/hpo_whitelist.json", api_key=None):
    ...
    async def fetch_global_top_n(self, session, hpo_list, top_k=3):
        ...
        try:
            async with session.get(self.pubcasefinder_url, params=params) as response:
            ...
            print(f"❌ Global Ranking Fetch Error: {e}")
        return []
    ...
    async def fetch_monarch(self, session, disease_id, original_name):
        try:
            ...
            print(f"❌ Monarch API 엔티티 분석 에러: {e}")
    ...
    async def fetch_pcf_disease_data(self, session, disease_id):
        ...
        try:
            ...
            print(f"❌ PCF Disease Data Fetch Error: {e}")
        return []
    ...
    async def fetch_pubcasefinder(self, session, hpo_list, top_k=2):
        ...
        for target in targets:
            ...
            tasks.append(session.get(self.pubcasefinder_url, params=params, timeout=5))
        try:
            responses = await asyncio.gather(*tasks, return_exceptions=True)
            combined_results = []
            for i, response in enumerate(responses):
                if isinstance(response, Exception) or response.status != 200: continue
                data = await response.json()
    ...
    async def gather_rag_data(self, pos_hpos, local_top_3):
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            combined_candidates = {d['id']: d for d in (local_top_3 + global_top_3)}.values()
            ...
            for disease in combined_candidates:
                ...
                if is_rare: non_pubmed_tasks["pubcasefinder"] = self.fetch_pubcasefinder(session, pos_hpos)
    ...
    def build_prompt(self, patient_input, rag_results):
        mapped_pos_hpos = self.format_hpo_list(patient_input.get('pos_hpos', []))
        mapped_neg_hpos = self.format_hpo_list(patient_input.get('neg_hpos', []))
