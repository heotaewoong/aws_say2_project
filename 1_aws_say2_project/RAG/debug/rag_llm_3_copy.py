...
class RareLinkHybridDualRAG:
    def __init__(self, orphadata_csv_path=None, hpo_dict_path="hpo_official.json", api_key=None):
        ...
        self.local_db = None
        # orphadata_csv_path 미지정 시 환경변수 또는 기본 경로 사용
        if orphadata_csv_path is None:
            orphadata_csv_path = os.environ.get('ORPHADATA_CSV_PATH', '/opt/python/orphadata.csv')
        ...
        # PubCaseFinder 실패 여부 추적 (호출 간 공유)
        self._pcf_failed = False
        self._pcf_fail_reason = ""

    # ── S3 캐시 헬퍼 (PubCaseFinder 결과 7일 캐싱) ──────────────────────
    def _pcf_cache_key(self, hpo_list: list) -> str:
        import hashlib
        sorted_str = ",".join(sorted(hpo_list))
        digest = hashlib.md5(sorted_str.encode()).hexdigest()
        return f"RAG/pcf_cache/{digest}.json"

    def _load_pcf_s3_cache(self, s3_key: str):
        try:
            ...
            if age_days < 7:
                print(f"✅ PubCaseFinder S3 캐시 히트 (age={age_days}d, key=...{s3_key[-8:]})")
                return data["results"]
            print(f"⚠️ PubCaseFinder S3 캐시 만료 (age={age_days}d) — 재호출")
        except Exception as e:
            if "NoSuchKey" not in str(e):
                print(f"⚠️ S3 캐시 로드 실패: {e}")
        return None

    def _save_pcf_s3_cache(self, s3_key: str, results) -> None:
        try:
            s3 = boto3.client("s3", region_name=os.environ.get("AWS_DEFAULT_REGION", "ap-northeast-2"))
            bucket = os.environ.get("S3_BUCKET", "say2-2team-bucket")
            s3.put_object(
                ...
                Key=s3_key,
                Body=json.dumps(
                    {"results": results, "cached_at": datetime.datetime.now().isoformat()},
                    ensure_ascii=False,
                ),
                ContentType="application/json",
            )
            print(f"💾 PubCaseFinder 결과 S3 캐시 저장 (key=...{s3_key[-8:]})")
        except Exception as e:
            print(f"⚠️ S3 캐시 저장 실패: {e}")
    ...
    async def fetch_global_top_n(self, session, hpo_list, top_k=3):
        # ── S3 캐시 우선 확인 (7일 TTL) ────────────────────────────────
        s3_key = self._pcf_cache_key(hpo_list)
        cached = self._load_pcf_s3_cache(s3_key)
        if cached is not None:
            return cached[:top_k]

        # ── 캐시 미스 → 실제 API 호출 (timeout 30s로 증가) ────────────
        hpo_query = ",".join(hpo_list)
        params = {"target": "orphanet", "format": "json", "phenotype": hpo_query}
        try:
            async with session.get(self.pubcasefinder_url, params=params, timeout=30) as response:
                if response.status == 200:
                    data = await response.json()
                    global_candidates = []
                    for item in data[:top_k]:
                        pcf_name = (
                            item.get("orpha_disease_name_en")
                            or item.get("omim_disease_name_en")
                            or item.get("name")
                        )
                    ...
                    # 성공하면 S3에 캐시 저장
                    self._save_pcf_s3_cache(s3_key, global_candidates)
                    return global_candidates
                else:
                    reason = f"HTTP {response.status}"
                    print(f"❌ PubCaseFinder API 응답 오류: {reason}")
                    self._pcf_failed = True
                    self._pcf_fail_reason = reason
        except asyncio.TimeoutError:
            reason = "API 응답 타임아웃 (30초 초과) — 서버 과부하 추정"
            print(f"❌ PubCaseFinder {reason}")
            self._log_error("fetch_global_top_n", hpo_query, Exception(reason))
            self._pcf_failed = True
            self._pcf_fail_reason = reason
        except Exception as e:
            self._log_error("fetch_global_top_n", hpo_query, e)
            self._pcf_failed = True
            self._pcf_fail_reason = str(e)
        return []
...
=== 8. RAG 검색 결과 (외부 API) ===
"""
        # PubCaseFinder 실패 시 Bedrock에게 명시적으로 알림
        if self._pcf_failed:
            prompt += (
                "\n⚠️ [데이터 수집 경고] PubCaseFinder (글로벌 HPO→희귀질환 매칭 API) 수집 실패.\n"
                f"실패 사유: {self._pcf_fail_reason}\n"
                "글로벌 API Top3 후보는 제공되지 않습니다. "
                "Orphanet·Monarch·PubMed·ClinicalTrials 데이터만으로 분석하십시오.\n"
                "rag_evidence 필드에 반드시 'PubCaseFinder 수집 실패 — 글로벌 API 교차검증 불가' 를 명시하십시오.\n\n"
            )
    ...
    def run_with_session_id(self, session_id: str) -> dict:
        """Phase 5: Aurora DB에서 Phase 1~4 결과를 읽고 RAG 파이프라인을 실행하여
        final_report 테이블에 저장.

        handler.py에서 호출:
            rag.run_with_session_id(session_id)

        Args:
            session_id: diagnosis_session의 UUID 문자열

        Returns:
            {"session_id": ..., "status": "completed", "elapsed_ms": ...}
        """
        import psycopg2
        ...
        db_params = {
            ...
            "database": os.environ.get("DB_NAME", "rarelink"),
        ...
        try:
            ...
            # ── 1. 세션 + 환자 기본정보 ─────────────────────────────────
            cur.execute("""
                SELECT ds.patient_id, ds.encounter_id,
                       pp.name_display, pp.age_years, pp.sex, pp.mrn
                FROM rarelinkai.diagnosis_session ds
                JOIN rarelinkai.patient_profile pp ON pp.patient_id = ds.patient_id
            ...
            cur.execute("""
                ...
                WHERE session_id = %s
            """, (session_id,))
            hpo_row = cur.fetchone() or {}
            pos_hpos = [h["hpo"] for h in (hpo_row.get("positive_hpo") or [])]
            neg_hpos = [h["hpo"] for h in (hpo_row.get("negative_hpo") or [])]

            # ── 3. Phase 3: 통합 랭킹 ────────────────────────────────────
            cur.execute("""
                SELECT ranking, lab_anomalies
                FROM rarelinkai.phase3_integrated_ranking
                WHERE session_id = %s
            """, (session_id,))
            rank_row = cur.fetchone() or {}
            ranking = rank_row.get("ranking") or []
            lab_anomalies = rank_row.get("lab_anomalies") or {}

            # ── 4. Phase 4: LLM 재랭킹 ───────────────────────────────────
            cur.execute("""
                SELECT reranked, reasoning_summary
                FROM rarelinkai.phase4_llm_rerank
                WHERE session_id = %s
            """, (session_id,))
            rerank_row = cur.fetchone() or {}
            reranked = rerank_row.get("reranked") or []
            ...
            """, (patient_id,))
            note_row = cur.fetchone() or {}
            chief_complaint = (note_row.get("note_text_ko") or "")[:500]

            # ── 6. Lab 결과 ──────────────────────────────────────────────
            cur.execute("""
                SELECT test_name_ko, value_numeric, value_unit, abnormal_flag
            ...
            for r in labs:
                flag = ""
                if r["abnormal_flag"] in ("H", "HH"):
                    flag = " (↑)"
                elif r["abnormal_flag"] in ("L", "LL"):
                    flag = " (↓)"
                lab_lines.append(
                    f"{r['test_name_ko']}: {r['value_numeric']} {r['value_unit'] or ''}{flag}"
                )
            lab_text = "\n".join(lab_lines) or "N/A"

            # ── 7. patient_input 빌드 ─────────────────────────────────────
            top_3_source = reranked[:3] if reranked else ranking[:3]
            top_3 = []
            for item in top_3_source:
                orpha = item.get("orpha", "")
                is_rare = "ORPHA" in str(orpha)
                top_3.append({
                    "id": orpha,
                    "name": item.get("name", ""),
                    "internal_genes": item.get("genes", "N/A"),
                    "is_rare": is_rare,
                })

            ranking_general = "\n".join([
                f"{i+1}. {r.get('name', '')} (LR={r.get('lr_score', 0):.2f})"
                for i, r in enumerate((ranking or [])[:10])
            ])
            ranking_rare = "\n".join([
                f"{i+1}. {r.get('orpha', '')} ({r.get('name', '')})"
                for i, r in enumerate(
                    [r for r in (ranking or []) if "ORPHA" in str(r.get("orpha", ""))][:5]
                )
            ]) or "해당 없음"

            patient_input = {
                "name":             sess.get("name_display") or "Unknown",
                "age":              sess.get("age_years") or 0,
                "sex":              sess.get("sex") or "N/A",
                ...
                "neg_hpos":         neg_hpos,
                "lab_data":         lab_text,
                "ranking_general":  ranking_general or "N/A",
                "ranking_rare":     ranking_rare,
                "internal_db_context": (
                    f"Phase 4 재랭킹 결과: {json.dumps(reranked[:3], ensure_ascii=False)}"
            ...
            # ── 8. RAG 파이프라인 ─────────────────────────────────────────
            generated_prompt = self.run_pipeline(patient_input)
            if not generated_prompt:
                raise RuntimeError("run_pipeline returned None — RAG 데이터 수집 실패")
            ...
            # 작업 C: Bedrock Prompt Management 우선 사용, 없으면 하드코딩 프롬프트 사용
            prompt_id = os.environ.get("BEDROCK_PROMPT_ID")
            active_system_prompt = self._get_bedrock_prompt(prompt_id) if prompt_id else SYSTEM_PROMPT
            ...
            if not m:
                raise ValueError("Bedrock 응답에서 JSON 객체를 찾을 수 없음")
            diagnosis_json = json.loads(m.group())

            # ── 11. HTML 리포트 생성 (markdown_report 컬럼) ───────────────
            html_report = self._json_to_html_report(diagnosis_json, patient_input)

            # ── 12. Self-check ────────────────────────────────────────────
            pmid_check = verify_pmids(raw_text, verbose=False)
            self_check = {
                "pmid_verification": pmid_check,
                "has_disclaimer":    bool(
                    diagnosis_json.get("clinical_notes", {}).get("disclaimer")
                ),
            }

            # ── 13. RAG citations ─────────────────────────────────────────
            rag_citations = {
                "pmid_valid":   pmid_check.get("valid_list", []),
                "pmid_invalid": pmid_check.get("invalid", []),
                "rag_evidence_summary": (
                    diagnosis_json.get("clinical_notes", {}).get("rag_evidence", "")
                ),
            }
            ...
            cur.execute("""
                ...
                ON CONFLICT (session_id) DO UPDATE SET
                    ...
                    rag_apis_used           = EXCLUDED.rag_apis_used,
                    self_check              = EXCLUDED.self_check,
                    llm_model               = EXCLUDED.llm_model,
                    total_inference_time_ms = EXCLUDED.total_inference_time_ms,
                    generated_at            = NOW()
            """, (
                session_id,
                json.dumps(diagnosis_json, ensure_ascii=False),
                html_report,
                json.dumps(rag_citations, ensure_ascii=False),
                ["pubcasefinder", "monarch", "pubmed", "clinicaltrials"],
            ...
            cur.execute("""
                ...
                SET status = 'completed', current_phase = NULL, completed_at = NOW()
                WHERE session_id = %s
            """, (session_id,))

            conn.commit()
            return {"session_id": session_id, "status": "completed", "elapsed_ms": elapsed_ms}
    ...
    def _json_to_html_report(self, diag: dict, patient_input: dict) -> str:
        """Bedrock JSON 출력을 의사용 HTML 리포트로 변환.
        final_report.markdown_report 컬럼에 저장됨.
        프론트엔드 dangerouslySetInnerHTML 또는 iframe으로 렌더링.
        """
        rec   = diag.get("recommendation", {})
        notes = diag.get("clinical_notes", {})
        conf  = diag.get("confidence_metrics", {})
        score = conf.get("overall_confidence_score", 0)
        score_color = "#0E8574" if score >= 0.8 else "#B45309" if score >= 0.6 else "#A32D2D"
        score_label = "High" if score >= 0.8 else "Medium" if score >= 0.6 else "Low"

        def li_items(lst):
            return "".join(f"<li>{item}</li>" for item in (lst or []))

        def section(title, content, title_color="#0C447C"):
            return (
                f'<div class="rs">'
                f'<div class="rt" style="color:{title_color}">{title}</div>'
                f'{content}'
                f'</div>'
            )
...
<meta charset="utf-8"/>
<style>
  body{{font-family:'Segoe UI',sans-serif;font-size:12px;color:#0A1628;margin:0;padding:16px;background:#F8FAFC;}}
  h1{{font-size:15px;border-bottom:2px solid #0C447C;padding-bottom:6px;color:#083158;}}
  .rs{{margin-bottom:12px;}}
  .rt{{font-size:10px;font-family:monospace;text-transform:uppercase;letter-spacing:.1em;margin-bottom:5px;}}
  ul{{margin:0;padding-left:16px;line-height:1.7;}}
  p{{margin:3px 0;line-height:1.6;}}
  .label{{font-size:10px;font-family:monospace;color:#64748B;margin-right:5px;}}
  .conf{{display:flex;align-items:center;gap:8px;padding:8px;background:white;border:1px solid #E2E8F0;margin-bottom:12px;}}
  .bar{{flex:1;height:6px;background:#E2E8F0;border-radius:3px;overflow:hidden;}}
  .bar-fill{{height:100%;border-radius:3px;}}
  .disclaimer{{margin-top:12px;padding:8px;background:rgba(180,83,9,.06);border:1px solid rgba(180,83,9,.2);font-size:10px;color:#B45309;}}
  .pcf-warn{{margin-bottom:10px;padding:8px 10px;background:#FFF7ED;border:1px solid #FDBA74;border-radius:4px;font-size:10px;color:#C2410C;}}
  .suf-dot{{display:inline-block;width:7px;height:7px;border-radius:50%;margin-right:4px;vertical-align:middle;}}
</style>
</head>
<body>
<h1>AI 진단 보조 리포트 · Rare-Link RAG Phase 5</h1>
<div style="font-size:10px;color:#64748B;margin-bottom:10px;">
  환자: {patient_input.get('name','')} · {patient_input.get('age','')}세 {patient_input.get('sex','')} · 생성: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M KST')}
</div>

{"<div class='pcf-warn'>⚠ PubCaseFinder (글로벌 HPO→희귀질환 매칭 API) 데이터 수집 실패 — " + self._pcf_fail_reason + ". 나머지 4개 API(Orphanet·Monarch·PubMed·ClinicalTrials)로 분석되었습니다.</div>" if self._pcf_failed else ""}

<div class="conf">
  <div style="min-width:60px;font-size:10px;color:#64748B;">종합 신뢰도</div>
  <div class="bar"><div class="bar-fill" style="width:{score*100:.0f}%;background:{score_color};"></div></div>
  <div style="font-size:12px;font-weight:600;color:{score_color};">{score*100:.0f}% ({score_label})</div>
</div>
{f'<div style="font-size:10px;color:#64748B;margin-bottom:10px;">{conf.get("rationale","")}</div>' if conf.get("rationale") else ""}

{section("임상 종합 소견", "".join([
    f'<p><span class="label">요약 ·</span>{notes.get("summary","")}</p>' if notes.get("summary") else "",
    f'<p><span class="label">Top 1 근거 ·</span>{notes.get("top1_reasoning","")}</p>' if notes.get("top1_reasoning") else "",
    f'<p><span class="label">감별진단 ·</span>{notes.get("differential_note","")}</p>' if notes.get("differential_note") else "",
    f'<p><span class="label" style="color:#6B21A8;">희귀질환 역학 ·</span><span style="color:#6B21A8;">{notes.get("epidemiology_note","")}</span></p>' if notes.get("epidemiology_note") else "",
]))}

{section("즉각 검사 권고", f'<ul>{li_items(rec.get("immediate_workup"))}</ul>') if rec.get("immediate_workup") else ""}
{section("전문과 의뢰 (MDT)", f'<ul>{li_items(rec.get("specialist_referral"))}</ul>', "#B45309") if rec.get("specialist_referral") else ""}
{section("치료 가이드라인", f'<ul>{li_items(rec.get("treatment_guideline"))}</ul>') if rec.get("treatment_guideline") else ""}
{section("유전자 검사", f'<ul>{li_items(rec.get("genetic_test"))}</ul>', "#6B21A8") if rec.get("genetic_test") else ""}
{section("추가 Lab 권고", f'<ul>{li_items(rec.get("additional_lab"))}</ul>') if rec.get("additional_lab") else ""}
{section("임상시험 정보", f'<ul>{li_items(rec.get("clinical_trial_info"))}</ul>') if rec.get("clinical_trial_info") else ""}
{section("RAG 근거 분석", "".join([
    f'<p><span class="label">DB·API 교차검증 ·</span>{notes.get("rag_evidence","")}</p>' if notes.get("rag_evidence") else "",
    f'<p><span class="label">케이스 비교 ·</span>{notes.get("case_comparison","")}</p>' if notes.get("case_comparison") else "",
])) if (notes.get("rag_evidence") or notes.get("case_comparison")) else ""}

<div class="disclaimer">⚠ AI-Assisted · {notes.get("disclaimer", "AI 결과는 진단 보조이며 최종 진단은 주치의의 임상 판단과 추가 검사 결과를 종합하여 확정합니다.")} [EU AI Act Art. 22]</div>
</body>
</html>"""
        return html

    # ── 작업 C: Bedrock Prompt Management 연동 ───────────────────────────
    def _get_bedrock_prompt(self, prompt_id: str) -> str:
        """Bedrock Prompt Management에서 시스템 프롬프트를 가져옴.
        BEDROCK_PROMPT_ID 환경변수가 없으면 코드 내 SYSTEM_PROMPT를 사용.
        """
        try:
            client = boto3.client(
                "bedrock-agent",
            ...
            for variant in response.get("variants", []):
                ...
                if text:
                    print(f"✅ Bedrock Prompt Management에서 시스템 프롬프트 로드 (ID: {prompt_id[:20]}…)")
                    return text
        except Exception as e:
            print(f"⚠️ Bedrock Prompt Management 로드 실패: {e} — 기본 프롬프트 사용")
