...
echo ""
echo "----- [1/5] aiohttp==3.9.5 설치 -----"
pip install "${PIP_COMMON_ARGS[@]}" "aiohttp==3.9.5"

echo ""
echo "----- [2/5] psycopg2-binary==2.9.9 설치 -----"
pip install "${PIP_COMMON_ARGS[@]}" "psycopg2-binary==2.9.9"

echo ""
echo "----- [3/5] pandas>=2.0,<3.0 설치 -----"
pip install "${PIP_COMMON_ARGS[@]}" "pandas>=2.0,<3.0"

echo ""
echo "----- [4/5] requests>=2.31.0,<3.0 설치 -----"
pip install "${PIP_COMMON_ARGS[@]}" "requests>=2.31.0,<3.0"

echo ""
echo "----- [5/5] typing_extensions (aiosignal 의존성) 설치 -----"
pip install "${PIP_COMMON_ARGS[@]}" "typing_extensions>=4.0"
